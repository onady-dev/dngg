...
- 최종 갱신: 2026-08-15
- **미머지 브랜치 1개**(`feat/season-phase1`)가 origin에 푸시돼 있다 — 다른 기기에서 `git fetch`로 바로 이어받을 수 있다.
- **로컬 `main`에 미푸시 커밋 3개가 있다** — 시즌제 설계·계획 문서(`9a0f260`, `3b60bf2`, `4881586`).
  `feat/season-phase1`에도 포함돼 있으므로 그 브랜치를 머지하면 함께 올라간다.
...
| 브랜치 | 상태 | 막고 있는 것 |
|---|---|---|
| `feat/season-phase1` | 구현·리뷰 완료 | **PR 생성**(gh 미설치)과 **브라우저 스모크**(사람) |
...
### `feat/season-phase1` — 시즌제 1단계 (구현 완료, PR 대기)
...
**남은 일 두 가지**
1. **PR 생성** — 이 머신에 `gh` CLI가 없어 만들지 못했다. 브랜치는 푸시돼 있다:
   https://github.com/onady-dev/dngg/pull/new/feat/season-phase1
2. **브라우저 클릭 스모크** — 시즌 드롭다운 전환, 새로고침 후 선택 유지(localStorage),
   시즌 관리 모달의 생성/이름변경/삭제/현재시즌 지정. 프론트에 테스트 러너가 없고
   자동화 도구도 없어 **자동 검증이 전혀 안 된 유일한 영역**이다.
...
**`feat/season-phase1` 머지 시**: 이 브랜치는 `backend/**`와 `frontend/**`를 모두 건드리므로
머지 커밋 하나로 두 잡이 함께 돌고 같은 sha로 핀된다 — **`workflow_dispatch`는 필요 없다.**
(2026-08-15 `deploy.yml` 확인: deploy 잡이 `needs.backend.result != 'failure' &&
needs.frontend.result != 'failure'` 조건이라 한쪽이라도 빌드에 실패하면 배포 자체가 스킵된다.)

다만 **프론트만 따로 배포하는 일은 절대 없어야 한다** — 구버전 백엔드에는 `/log/rankings`가
없어 랭킹 페이지가 404로 통째로 실패한다. `/season` 404는 try/catch로 '전체' 폴백되므로 선수
상세는 살아남는다. 즉 피해는 랭킹 한 곳에 집중되고 전면적이다. **롤백도 동반 롤백이어야
한다** — 백엔드만 이전 sha로 되돌리면 같은 404가 재현된다. 배포 후 `/rankings`와
`/player/:id`를 직접 열어볼 것.
