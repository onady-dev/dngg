...
- 최종 갱신: 2026-08-17
- **진행 중인 브랜치 없음.** 시즌제 1단계는 PR #3으로 머지·운영 배포 완료(2026-08-17, `35a3153`).
- `origin/feature/privacy-page`와 `origin/feature/marketing-relaunch`는 **이미 main에 다 들어갔다**
  ...
  지워도 된다. `feat/season-phase1`도 머지 완료라 삭제 가능하다.
...
**⚠️ 브라우저 클릭 스모크를 하지 않고 배포했다 (인지된 선택).** 시즌 드롭다운 전환,
새로고침 후 선택 유지(localStorage), 시즌 관리 모달의 생성/이름변경/삭제/현재시즌 지정 —
이 UI 조작 경로들은 **한 번도 사람 손으로 눌러본 적이 없다.** 프론트에 테스트 러너가 없고
자동화 도구도 없어 자동 검증이 불가능했다. API·DB 레벨과 페이지 200 응답까지만 확인했다.
**시즌 기능에서 이상이 보고되면 여기부터 의심할 것.**
