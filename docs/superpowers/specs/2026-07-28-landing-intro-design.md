...
1. `public/manual/screenshots/`의 샷들은 **실제 사용자 이름**을 그대로 담고 있다
   (실명 여러 건). 매뉴얼에 이미 공개돼 있긴 하지만 랜딩은
...
이 작업은 **그중 어느 코드 파일도 건드리지 않는다** (OG 카드를 범위에서 뺀 이유가 이것이다).
`page.tsx`는 GA 브랜치가 손대지 않으므로 코드 충돌은 없고, 머지 순서는 자유다.

**다만 문서는 충돌한다.** 두 브랜치가 `handoff.md`의 "남은 TODO"·"미해결 이슈"와
`docs/featurelist.md`의 체크박스 블록에서 같은 줄을 고친다(`git merge-tree`로 확인).
나중에 머지하는 쪽이 이 두 파일의 충돌을 해결해야 한다 — 텍스트 충돌이라 해결은 간단하다.
