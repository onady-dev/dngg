...
## 9. 운영 DB 확인 결과 (2026-08-14 확인 완료)

가정하지 않고 운영 DB에서 직접 확인했다.

### 9.1 `logitem` — 전 그룹 동일 시드

11개 그룹 모두 아래 10개 항목을 이름·value까지 똑같이 갖고 있다. 사용자가 정의한
커스텀 항목은 존재하지 않는다.
...
| `2점`, `자유투2점` | 2 |
| `자유투1점` | 1 |
| `리바`, `블록`, `스틸`, `어시`, `파울`, `턴오버` | 0 |

확인된 사실:

- **능력치 6축의 문자열 매칭이 전부 정상 동작한다** — `리바`→'리바', `어시`→'어시',
  `스틸`/`블록`, `턴오버`/`파울`, 외곽→'3점'. `mode`는 항상 `'basketball'`이며
  `'dynamic'` 경로는 운영에서 사용되지 않는다.
- **비득점 항목의 `value`가 전부 0이다.** 부록의 팀 점수 계산 가정(2단계)이 확증됐다.
- 자유투는 `자유투1점`·`자유투2점` 두 개이고 `includes('자유투')` 필터가 둘 다 잡는다.
- 참고: `'자유투2점'.includes('2점')`은 `true`지만, 이 매칭은 `BASKETBALL_AXES`의
  `present` 체크에만 쓰이고 raw 계산에는 쓰이지 않아 무해하다.

### 9.2 데이터 규모 (isDeleted=false 그룹 4개)

| 그룹 | 선수 | 경기 | 삭제 경기 | 로그 |
|---|---|---|---|---|
| 스내치 (id 1) | 44 | 45 | 30 | 6,734 |
| 농우회 (id 10) | 30 | 10 | 2 | 1,338 |
| Breakers (id 14) | 34 | 1 | 1 | 0 |
| 경북포항농구/온에어 (id 16) | 0 | 0 | 0 | 0 |

최대 6.7천 로그 규모이므로 **서버 집계 쿼리에 별도 튜닝은 불필요하다.** 1.5의
`Game(groupId, seasonId)` 인덱스만으로 충분하다.

### 9.3 3.3에서 고치는 두 동작의 실제 영향 (스내치 기준)

- **삭제 경기 제외**: 로그 15건만 제외된다(전체의 0.2%, 득점 −24). 삭제된 경기 30개
  중 대부분이 기록이 없는 빈 경기였다 — 우려보다 영향이 훨씬 작다.
- **자유투 포함**: 득점 +177점(약 +4%). 농우회는 +46점. 이쪽이 사용자에게 체감되는
  변화이므로, 배포 후 "득점이 늘었다"는 문의가 올 수 있음을 감안한다.
...
- **점수 계산**: 경기 점수는 저장된 값이 아니라 해당 팀 `InGamePlayer`의 로그
  `logitem.value` 합이다. 이는 비득점 항목의 `value`가 0이라는 가정 위에 서 있는데,
  9.1에서 **운영 DB로 확인 완료**했다(리바·블록·스틸·어시·파울·턴오버 모두 0).
