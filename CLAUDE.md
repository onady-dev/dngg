...
## 트래픽 급증 대응 (⚠️ 마케팅 확대 예정)

**사이트가 느려지거나 트래픽이 늘었다는 이야기가 나오면 `docs/runbooks/scaling.md`의 맨 앞 "🚨 트래픽이 늘었다" 절을 먼저 읽을 것.** 상태 확인 명령, 판단 기준, 조치가 복붙 가능한 형태로 정리되어 있다. 아래는 그 문서를 안 읽고 잘못된 방향으로 가는 것을 막기 위한 핵심 3줄이다.

- **이 서비스는 CPU가 남는 상태에서 먼저 느려진다.** 2026-08-10 부하 테스트에서 동시 100명에 CPU 12.7%·크레딧 무소모인데 p95가 7.2초였다. 병목은 **DB 커넥션 풀**이었고, `DB_POOL_MAX`를 10 → 40으로 올리자 동시 50명 p95가 **3.118초 → 37ms**가 됐다. **p95가 나쁜데 CPU가 여유로우면 인스턴스를 키우지 말고 풀부터 볼 것.** (서버 `.env` 수정 + `docker compose up -d backend`, 재배포 불필요. 상한은 Postgres `max_connections`=100)
- **전역 로그인 rate limit이 IP별이 아니라 전역 단일 버킷(5분 300회)이다.** 스파이크가 스스로를 429로 잠글 수 있다. `SITEWIDE_LOGIN_THROTTLE_LIMIT`을 서버 `.env`에서 올리면 재배포 없이 풀린다.
- **보안 그룹의 SSH(22)를 좁히지 말 것.** CI 배포가 SSH로 서버에 붙는다. 좁히면 이미지는 빌드·푸시되지만 서버에 반영되지 않아 **초록불인 채로 구버전이 계속 돈다** (2026-08-10에 실제로 겪음).

수직 확장(t3.small, +$9/월)은 동시 100명에서 p95 10ms·실패 0건으로 검증됐고 절차도 리허설을 마쳤다 — 다만 다운타임 2~3분이 들고, **`docker compose stop`으로 내린 컨테이너는 `restart: always`로 살아나지 않으므로 기동 후 `docker compose up -d`가 필요하다.**
